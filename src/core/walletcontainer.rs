...
enum WalletType {
    Online(Wallet<AnyDatabase>, AnyBlockchain),
...
impl WalletContainer {
    ...
    pub fn new_online(wallet : Wallet<AnyDatabase>, blockchain : AnyBlockchain) -> WalletContainer{
        return WalletContainer{typ : Online(wallet, blockchain)}
    }
    ...
    pub fn get_online_wallet(&self) -> Result<(&Wallet<AnyDatabase>, &AnyBlockchain),Box<dyn Error>>{
        if let Online(wallet,blockchain) =  &self.typ {
            return Ok((wallet, blockchain));
        }
    ...
    pub fn get_balance(&self) -> Result<u64, bdk::Error>{
        return match &self.typ {
            Online(wallet,_) => wallet.get_balance(),
    ...
    pub fn get_address(&self, address_index: AddressIndex) -> Result<AddressInfo, bdk::Error> {
        return match &self.typ {
            Online(wallet,_) => wallet.get_address(address_index),
    ...
    pub fn list_transactions(&self, include_raw: bool) -> Result<Vec<TransactionDetails>, bdk::Error>{
        return match &self.typ {
            Online(wallet,_) => wallet.list_transactions(include_raw),
